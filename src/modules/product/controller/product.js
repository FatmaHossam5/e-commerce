    ...
    async (req, res, next) => {
        if (!req.files?.length) {
            ...
            for (const file of req.files) {
                const { secure_url, public_id } = await cloudinary.uploader.upload(file.path, { folder: `category/products/${name}` })
...
export const products = asyncHandler(
   ...
   ;
 return  res.status(200).json({ message: "Done", finalProducts })
}
